...
describe("Player Insights - getPlayerScoreTrends", () => {
  ...
  test("normal case: returns empty trend arrays", async () => {
    const { receiverCaller } = await createInsightsCallers(ids!);
    const player = await receiverCaller.player.create({
      name: "Empty",
      imageId: null,
    });
    const result = await receiverCaller.newPlayer.getPlayerScoreTrends({
      type: "original",
      id: player.id,
    });
  ...
  test("best case: returns populated trend arrays", async () => {
    const { ownerCaller } = await createInsightsCallers(ids!);
    const seeded = await seedInsightsHistory(ids!);
    const result = await ownerCaller.newPlayer.getPlayerScoreTrends({
      type: "original",
  ...
  test("worst case: throws for missing player", async () => {
    const { receiverCaller } = await createInsightsCallers(ids!);
    await expect(
      receiverCaller.newPlayer.getPlayerScoreTrends({
        type: "original",
        id: 99999999,
      }),
