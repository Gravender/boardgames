...
- `getPlayerToShare`
  - Confirm and enforce contract policy:
    - original-only (recommended if only owned items are shareable), or
    - discriminated original/shared if product needs both.
...
3. Implement service-level permission checks and transaction orchestration in `player.service.ts`.
4. Preserve post-commit side effects (analytics and file deletion) in service layer.
5. Wire router procedures in `player/player.router.ts`.
6. Keep old `players.ts` mutation procedures as wrappers during transition.
7. Update Next.js mutation hooks and query invalidation paths.
